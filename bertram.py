...
from transformers import BertModel, BertConfig, RobertaModel, BertTokenizer, RobertaTokenizer, DistilBertTokenizer, BertPreTrainedModel, \
    RobertaConfig, PreTrainedTokenizer, PreTrainedModel
from transformers.modeling_outputs import BaseModelOutput

from modeling_vgcn_bert import VGCNBertModel
...
class Bertram(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        if requires_context(self.bertram_config.mode):
            ...
            sequence_output = output.last_hidden_state if hasattr(output, 'last_hidden_state') else output[0]

            # 确保 sequence_output 是张量而不是字符串
            # if isinstance(sequence_output, BaseModelOutput):
            #     sequence_output = sequence_output.last_hidden_state  # 获取张量
            # else:
            #     print(f'type(sequence_output) = ', type(sequence_output))
    ...
    def _get_mask_output(seq, indices):
        ...
        if isinstance(seq, str):
            raise TypeError(f"Expected tensor for sequence_output, but got {type(seq)}")
...
# class BertramForVGCNBert(Bertram):


MODELS = {
    'bert': (BertModel, BertTokenizer, Bertram),
    'roberta': (RobertaModel, RobertaTokenizer, BertramForRoberta),
    'vgcn_bert': (VGCNBertModel, DistilBertTokenizer, VGCNBertConfig)  # add VGCNBert
}
...
class BertramWrapper:
    ...
    def __init__(self, model_path: str, device: str = 'gpu'):
        ...
        # print(f'current transformer_cls {bertram_config.transformer_cls}')
        # print(f'current bertram_cls {bertram_cls}')
    ...
    def add_word_vectors_to_model(self, words_with_contexts: Dict[str, List[str]], tokenizer: PreTrainedTokenizer,
        ...
        for word, embedding in embeddings.items():
            ...
            _get_embeddings_module(model).word_embeddings.weight[word_id].data.copy_(embedding.detach())
...
def _get_embeddings_module(model):
    # if hasattr(model, 'embeddings') and hasattr(model.embeddings, 'word_embeddings'):
        # print('in get_embeddings_module, compatible to VGCNBERT')
        # return model.embeddings  # For VGCNBertModel compatibility
    if hasattr(model, 'vgcn_bert'):
        print('in get_embeddings_module, compatible to VGCNBERT')
        return model.vgcn_bert.embeddings.word_embeddings
